...
This release hardens Cassetta into a local-first behavior cassette and regression gate for MCP and JSON-RPC workflows. It adds typed cassette-format errors with line and sequence context, request-aware offline replay, stable CLI exit codes, SARIF-shaped diff output, stronger stdio failure handling, and a repository-level CLI smoke path. It also ships the architecture contract, ADRs, threat model, verification record, product README, CI quality gate, and tag-triggered release workflow.
