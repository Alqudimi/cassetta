...
const redactValue = (
  value: JsonValue,
  policy: Required<RedactionPolicy>,
  path = "$"
): JsonValue => {
  if (Array.isArray(value))
    return value.map((item, index) =>
      redactValue(item, policy, `${path}[${index}]`)
    );
  if (typeof value === "string") {
    return policy.valuePatterns.some(pattern => pattern.test(value))
      ? policy.replacement
      : value;
  }
  if (!isObject(value)) return value;
...
export const normalizeMessage = (message: ProtocolMessage): ProtocolMessage =>
  normalizeValue(message, "$") as ProtocolMessage;
...
export const diffEntries = (
  expected: CassetteEntry[],
  actual: CassetteEntry[]
): DiffLine[] => {
