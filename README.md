...
The domain core is independent from transports and presentation. The stdio adapter owns process lifetime and JSONL framing; the core owns evidence semantics; the CLI owns filesystem boundaries, reports, and exit codes.

```text
local JSON-RPC process → capture → normalize → redact → cassette.jsonl
                                                       ├─ offline replay
                                                       ├─ deterministic diff
                                                       └─ CI/SARIF gate
```
...
Cassetta does not replace the [official MCP Inspector](https://modelcontextprotocol.io/docs/2026-07-28/tools/inspector), [MCPJam](https://github.com/MCPJam/inspector), Pact, hosted observability, LLM-as-judge evaluation, or a complete MCP conformance suite. Streamable HTTP, OAuth workflows, signed manifests, schema-aware assertions, and semantic evaluation are intentionally outside the first local stdio MVP.
